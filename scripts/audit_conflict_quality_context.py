"""Audit conflict_quality_context coverage and future-label hygiene."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Audit conflict_quality_context in evidence packs.")
    ...
    csv_path = out_dir / f"{prefix}_conflict_quality_context_audit.csv"
    md_path = out_dir / f"{prefix}_conflict_quality_context_audit.md"
...
def _audit(rows: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    for index, row in enumerate(rows):
        context = str(row.get("conflict_quality_context") or "none")
...
def _write_report(path: Path, detail: pd.DataFrame, evidence_path: Path) -> None:
    if detail.empty:
        path.write_text("# Conflict Quality Context Audit\n\n_无数据_\n", encoding="utf-8")
        return
    block_counts = (
        detail.groupby(["valid_block", "task_mode"], dropna=False)
        .agg(
    ...
    lines = [
        "# Conflict Quality Context Audit",
        "",
        "本报告只审计 evidence pack 中的冲突质量上下文，不构成投资建议。",
        "",
        ...
        "## Rules",
        "",
        "- `conflict_quality_context` 必须包含 `walk_forward_prior_only` 才能给 Agent 使用。",
